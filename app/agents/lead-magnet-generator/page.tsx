...
export default function LeadMagnetGeneratorPage() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.user) {
        ...
        trackAgentEvent({ eventType: 'signup', agentId: 'lead-magnet-generator', userId: data.user.id });
      }
    } catch (err) {
      console.error(err);
    }
  ...
  const handleGenerate = async () => {
    ...
    try {
      trackAgentEvent({
        ...
        agentId: 'lead-magnet-generator',
        userId: user?.id || null,
        payload: { leadMagnetGoal: form.leadMagnetGoal, tone: form.tone, industry: form.industry },
      });
  ...
  const fullCopyText = useMemo(() => {
    ...
    const lines = [
      ...
      '',
      'FORMAT VARIANTS:',
      ...output.formatVariants.flatMap(v => [`- ${v.format}: ${v.hook}`, ...v.outline.map(o => `  • ${o}`)]),
      '',
      'LEAD MAGNET DRAFT:',
              ...
              <div className="grid grid-cols-1 gap-4">
                ...
                <label className="space-y-2">
                  ...
                  <input
                    ...
                    placeholder="e.g., improving demo-to-close, pipeline quality, churn reduction"
                    className="w-full bg-black border border-[#1f1f1f] rounded py-3 px-4 focus:outline-none focus:border-[#333] text-sm font-medium"
                  />
                </label>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <label className="space-y-2">
                    <span className="text-[10px] font-black uppercase tracking-[0.2em] text-[#666]">Brand style (optional)</span>
                    <input
                      value={form.brandStyleNotes}
                      onChange={e => setForm(prev => ({ ...prev, brandStyleNotes: e.target.value }))}
                      placeholder="e.g., crisp, no fluff"
                      className="w-full bg-black border border-[#1f1f1f] rounded py-3 px-4 focus:outline-none focus:border-[#333] text-sm font-medium"
                    />
                  </label>
                </div>
            ...
            <div className="p-6 rounded-lg border border-[#1f1f1f] bg-[#0a0a0a] space-y-5">
              ...
              {!output && (
                <div className="p-10 rounded border border-dashed border-[#1f1f1f] bg-black text-center space-y-3">
                  ...
                  <p className="text-sm text-[#888] font-medium">Fill the inputs and click Generate to get a full lead magnet draft + conversion copy.</p>
                  <p className="text-[11px] text-[#666]">Optimized for persona relevance and conversion-focused framing.</p>
                </div>
              )}
          ...
          <div className="relative bg-[#0a0a0a] w-full max-w-sm p-10 rounded-lg border border-[#1f1f1f] space-y-10 shadow-2xl">
            ...
            <div className="space-y-4">
              <button
                type="button"
                onClick={() => {
                  trackAgentEvent({ eventType: 'signup_intent_google', agentId: 'lead-magnet-generator', userId: user?.id || null });
